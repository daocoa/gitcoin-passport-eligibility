...
contract ModuleTest is DeployImplementation, Test {
  ...
  IGitcoinResolver public constant GITCOIN_RESOLVER = IGitcoinResolver(0xc94aBf0292Ac04AAC18C251d9C8169a8dd2BBbDC);
  ...
  function _deployInstance(uint256 _criterion) internal returns (GitcoinPassportEligibility) {
    // prepare the script with the necessary argss
    deployInstance.prepare(false, targetHat, address(implementation), address(GITCOIN_PASSPORT_DECODER), _criterion);
  ...
  function createScoreAttestation(address _recipient, uint256 _score, uint64 _expiry) public returns (bytes32) {
...
contract Deployment is ModuleTest {
  ...
  function test_gitcoinPassportDecoder() public {
    assertEq(address(instance.GITCOIN_PASSPORT_DECODER()), address(GITCOIN_PASSPORT_DECODER));
  }
...
contract IsHuman is ModuleTest {
  function test_exists_notRevoked_notExpired(uint256 _score, uint256 _criterion) public {
  ...
  function test_exists_revoked_notExpired(uint256 _score, uint256 _criterion) public {
    scoreCriterion = bound(_criterion, 0, 100);
    instance = _deployInstance(scoreCriterion);
    if (scoreCriterion == 0) scoreCriterion = 20;
  ...
  function test_exists_notRevoked_expired(uint256 _score, uint256 _criterion) public {
    scoreCriterion = bound(_criterion, 0, 100);
    instance = _deployInstance(scoreCriterion);
    if (scoreCriterion == 0) scoreCriterion = 20;
    score = bound(_score, scoreCriterion, 100) * 10 ** DECIMALS;
...
contract GetWearerStatus is ModuleTest {
  ...
  function test_insufficientScore(uint256 _score, uint256 _criterion) public {
    scoreCriterion = bound(_criterion, 0, 100);
    instance = _deployInstance(scoreCriterion);
    if (scoreCriterion == 0) scoreCriterion = 20;
